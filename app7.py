...
def save_uploaded_file(directory, file):
    ...
    #2.디렉토리가 존재하니짜 파일을 이 디렉토리 안에 저장한다
    with open(os.path.join(directory,file.name),'wb') as f:
        f.write(file.getbuffer())

    #3. 파일저장에 성공했으니까 유저한테 웹에서 보여주자
    return st.success (directory+'에'+file.name+' 파일이 잘 저장되었다')
...
def main():
    ...
    if choice == 'image':
        ...
        if file is not None:
            ...
            #파일명을 일관성있게 관리 할수 있도록
            #회사에 들어가면 파일명 작명 규칙대로 바꿔 준다
            #현재시간을 조합하여 파일명을 만든다
            #유니크하게 파일명을 지을수 있다
            ...
            #파일이 업로드 되었을때만 파일을 저장하는 코드를 작성한다

            save_uploaded_file('tmp',file)

            #웹화면에 저장된 이미지 파일을 보여주자
            img=Image.open(file)
            st.image(img)
